...
$phone     = get_user_meta( $user_id, 'user_phone', true ) ?: __( 'For example +380501234567', 'krop' );
$email = $user_data->user_email ?: __( 'For example mail@example.com', 'krop' );
$display_name = $user_data->display_name ?: __( 'For example: Ivan Franko', 'krop' );
...
<div class="personal__content__form">
    <form action="" id="profile_form">
        <div class="login__form__text personal__content__form__text"><?php _e( 'Surname and first name', 'krop' ); ?></div>
        <input type="text" name="user_display_name" class="login__form__input"
               placeholder="<?php echo $display_name; ?>">
        <div class="login__form__text"><?php _e( 'Email', 'krop' ); ?></div>
        <input type="email" class="login__form__input email"
               pattern="(?![_.-])((?![_.-][_.-])[\w.-]){0,63}[a-zA-Z\d]@((?!-)((?!--)[a-zA-Z\d-]){0,63}[a-zA-Z\d]\.){1,2}([a-zA-Z]{2,14}\.)?[a-zA-Z]{2,14}"
               name="user_email" autocomplete="email"
               placeholder="<?php echo $email; ?>" <?php echo $transitive_email ? 'value="'.$transitive_email.'"': ''; ?>>
		...
		if ( $transitive_email ) {
			?>
            <div class="login__form__input-error need-confirmation login__form__input-error--show" aria-hidden="true">
                <span><?php _e( 'The email will not be changed until it is confirmed.', 'krop' ); ?> <a data-confirm-type="email"
                            href="<?php echo add_query_arg( array( 'send_confirm_again' => 1 ), get_the_permalink() ); ?>"><?php _e( 'Didn\'t receive a confirmation letter?', 'krop' ); ?></a>
                </span>
            </div>
			<?php
		}
		?>
        <div class="login__form__text"><?php _e( 'Phone number', 'krop' ); ?></div>
        <input type="tel" class="login__form__input phone"
               name="user_phone"
               placeholder="<?php echo $phone; ?>" <?php echo $transitive_phone ? 'value="'.$transitive_phone.'"': ''; ?>>
        <div class="login__form__input-error invalid_phone" aria-hidden="true">
			<?php _e( 'Invalid phone number', 'krop' ); ?>
        </div>
        <?php
        $user = wp_get_current_user();
        do_action('qm/debug', array(get_user_meta( $user->ID, 'transitive_login', true ), get_user_meta( $user->ID, 'transitive_email', true ), get_user_meta( $user->ID, 'transitive_phone', true )));
        if ( $transitive_phone ) {
	        ?>
            <div class="login__form__input-error need-confirmation login__form__input-error--show" aria-hidden="true">
                <span><?php _e( 'The phone number will not be changed until it is confirmed.', 'krop' ); ?> <a data-confirm-type="phone"
                            href="<?php echo add_query_arg( array( 'send_confirm_again' => 2 ), get_the_permalink() ); ?>"><?php _e( 'Didn\'t receive a confirmation SMS?', 'krop' ); ?></a>
                </span>
            </div>
	        <?php
        }
        ...
        <div class="d-flex flex-dir-column position-relative div-password-field">
            <input type="password" class="login__form__input mobile-placeholder"
                   name="user_password" autocomplete="new-password"
                   data-moblile-placeholder-text="<?php _e( 'New password', 'krop' ); ?>"
                   data-default-placeholder-text="<?php _e( 'New password for account', 'krop' ); ?>">
            <div class="login__form__input__password-visible password-visible-switcher">
                <span class="login__form__input__icon form-password-visible-icon login__form__input__icon__hide">
        ...
        <input type="hidden" name="_wpnonce" value="<?php echo wp_create_nonce( 'account-editing' ); ?>">
        <input type="hidden" name="action" value="account_editing">
        <button type="submit" class="btn btn--primary login__form__btn personal__content__form__btn">
			<?php _e( 'Approve new data', 'krop' ); ?>

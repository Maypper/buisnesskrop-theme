...
<div class="modal category__modal add-brand__modal" data-modal="modal-category">
    <div class="category__head d-flex justify-content-between w-100">
        <div class="category__title">
			<?php _e( 'Select the desired brand or business category from the list', 'krop' ); ?>
        </div>
        <a class="category__close-modal">
    ...
    <div class="row category__list">
		<?php
		foreach ( $brands_cats as $key => $brands_cat ) {
            if ($post_id) {
                $active_cats = get_the_terms( $post_id, 'category-brands' );
                $is_checked = false;
                foreach ($active_cats as $active_cat) {
                    if ($active_cat->term_id == $key) {
	                    $is_checked = ' checked';
                    }
                }
            }
			echo '<div class="category__item"><label class="checkbox' . $is_checked . '"><input type="checkbox"' . $is_checked . ' name="category[]" value="' . $key . '">' . $brands_cat . '</label></div>';
		}
		?>
    </div>
    <button type="button" class="category__button-submit btn btn--primary">
		<?php _e( 'Confirm categories', 'krop' ); ?>
